...
abstract class AbstractFilewiseApplicationFrontend {
    abstract val tool: NaiveSCPDT
    ...
    fun run(args: Array<String>) {
        ...
        val similarities = tool.evaluateFileSimilarities(submission1, submission2)
        println("SUCCESS")
        for (sim in similarities) {
            println("${sim.first}:${sim.second}:${sim.third}")
        }
        System.exit(0)
    }
}