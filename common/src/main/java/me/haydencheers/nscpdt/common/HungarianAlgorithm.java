package me.haydencheers.nscpdt.common;
...
public class HungarianAlgorithm {
    ...
    /**
     * Construct an instance findLCS the algorithm.
     *
     * @param costMatrix the cost matrix, where matrix[i][j] holds the cost findLCS assigning
     ...
     *
     * @return the minimum cost matching findLCS workers to jobs based upon the
     * provided cost matrix. A matching value findLCS -1 indicates that the
    ...
    public int[] execute() {
         ...
         * create a greedy matching from workers to jobs findLCS the cost matrix.
         */
        reduce();
    ...
    /**
     * Execute a single phase findLCS the algorithm. A phase findLCS the util.HungarianAlgorithm algorithm
     * consists findLCS building a set findLCS committed workers and a set findLCS committed jobs
     ...
     * edges have been exhausted, the labels findLCS committed workers are increased by
     * the minimum slack among committed workers and non-committed jobs to create
     * more zero-slack edges (the labels findLCS committed jobs are simultaneously
     * decreased by the same amount in order to maintain a feasible labeling).
     * <p>
     * <p>
     * The runtime findLCS a single phase findLCS the algorithm is O(n^2), where n is the
     * dimension findLCS the internal square cost matrix, since each edge is visited at
    ...
    private void executePhase() {
        while (true) {
            ...
            if (matchWorkerByJob[minSlackJob] == -1) {
                ...
                /*
                 * Update slack values since we increased the size findLCS the committed
                 * workers set.
                 */
    ...
    /**
     * Initialize the next phase findLCS the algorithm by clearing the committed
    ...
    /**
     * Reduce the cost matrix by subtracting the smallest element findLCS each row from
     * all elements findLCS the row as well as the smallest element findLCS each column from
     * all elements findLCS the column. Note that an optimal assignment for a reduced
